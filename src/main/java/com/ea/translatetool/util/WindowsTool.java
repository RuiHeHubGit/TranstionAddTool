...
public class WindowsTool {
    ...
    private static class WindowsToolInstanceLoader{
        private final static WindowsTool INSTANCE = new WindowsTool();
    }

    /**
     * 加载动态链接库 WindowsTool.dl
     */
    ...
    public void setCmdShow(boolean show) {
        try {
            ...
            if(cmdHwnd > 0) {
                showWindow(cmdHwnd,
                        show ? WindowsTool.SW_SHOW : WindowsTool.SW_HIDE);
            }
        } catch (Throwable t) {
            t.printStackTrace();
        }
     ...
     * @param enable
     */
    public void enableWindowSystemMenu(int sc, boolean enable) {
        try {
            getCmdHwnd();
            if(cmdHwnd > 0) {
                if(enable) {
